...
public class BookmarkRepository {
    ...
    public List<Bookmark> findAll() {
        ...
        return jdbcTemplate.query(sql, BookmarkRowMapper.INSTANCE);
    }

    public Optional<Bookmark> findById(Long id) {
        String sql = "select id, title, url, created_at from bookmarks where id = ?";
        try{
            Bookmark bookmark = jdbcTemplate.queryForObject(sql, BookmarkRowMapper.INSTANCE, id);
    ...
    public void update(Bookmark bookmark) {
        ...
        if(count == 0) throw new RuntimeException("Bookmark not found");
    }
